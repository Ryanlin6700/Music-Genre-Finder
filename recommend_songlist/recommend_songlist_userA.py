...
def find_similar_songs(file_name):
    ...
    # 讀預測歌曲特徵csv
    userdf_3s2 = pd.read_csv(f'./musicfile/csv/3s/{file_name}/{file_name}.csv', index_col=False)
    ...
    series = sim_df_names[file_name].sort_values(ascending = False) # 相關性高到低
    series = series.drop(file_name) # 排除自己對應的係數=1
    # series = series.iloc[0:5,] # 篩選前五筆
...
find_similar_songs(file_name) # call function
