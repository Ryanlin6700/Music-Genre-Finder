...
def find_similar_songs(file_name):
    ...
    data = pd.read_csv('./static/finalcsv/finalcsv30s.csv',index_col=False)
    data = data.dropna()
    ...
    # 讀預測歌曲特徵csv
    userdf_3s2 = pd.read_csv(f'./static/musicfile/csv/3s/{file_name}/{file_name}.csv', index_col=False)
