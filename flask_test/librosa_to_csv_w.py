...
1. 修改路徑結構 musicfile/
    mp4/{file_name}/{file_name}.mp4 
    wav/3s/{file_name}/{file_name}-{}-{}.wav    csv/3s/{file_name}/{file_name}.csv
       30s/{file_name}/{file_name}-{}.wav          30s/{file_name}/{file_name}.csv 
2. 音檔分段改為，去頭 60s - 去尾 30s。
2. 選特徵：改 feature_dir 字典內容 選取需要轉出的特徵。
...
def Video_to_csv(file_name):
    # --------初始化--------
    # 路徑設定 & 確認資料夾，沒有則建立
    # mp4 路徑 & 轉出 wav 音檔路徑
    mp4_folder = f'./static/musicfile/mp4/{file_name}'  # mp4 來源路徑
    wav_parentfolder = './static/musicfile/wav'        # 給 wav 輸出目錄的上一層目錄
    wav_outputpath = f'./static/musicfile/wav/{file_name}/'  # 給 wav 輸出目錄
    # 如果沒有資料夾建立資料夾
    if not isdir( mp4_folder):
        mkdir( mp4_folder)
    if not isdir(wav_parentfolder):
        mkdir(wav_parentfolder)
    if not isdir(wav_outputpath):
        mkdir(wav_outputpath)
    
    # wav 分割成 3s 存放路徑
    wav_parentfolder_3s = './static/musicfile/wav/3s'
    wav_outputpath_3s = f'./static/musicfile/wav/3s/{file_name}'
   
    if not isdir(wav_parentfolder_3s):
        mkdir(wav_parentfolder_3s)
    if not isdir(wav_outputpath_3s):
        mkdir(wav_outputpath_3s)
        
    # wav 分割成 30s 存放路徑
    wav_parentfolder_30s = './static/musicfile/wav/30s'
    wav_outputpath_30s = f'./static/musicfile/wav/30s/{file_name}'
    
    if not isdir(wav_parentfolder_30s):
        mkdir(wav_parentfolder_30s)
    if not isdir(wav_outputpath_30s):
        mkdir(wav_outputpath_30s)
        
    # 特徵 csv 存放路徑 
    csv_folder ='./static/musicfile/csv'
    csv_parentfolder_3s = './static/musicfile/csv/3s'
    csv_parentfolder_30s = './static/musicfile/csv/30s'
    csv_outputpath_3s = f'./static/musicfile/csv/3s/{file_name}'
    csv_outputpath_30s = f'./static/musicfile/csv/30s/{file_name}'
    if not isdir(csv_folder):
        mkdir(csv_folder)
    
    if not isdir(csv_parentfolder_3s):
        mkdir(csv_parentfolder_3s)
    if not isdir(csv_parentfolder_30s):
        mkdir(csv_parentfolder_30s)
        
    if not isdir(csv_outputpath_3s):
        mkdir(csv_outputpath_3s)
    if not isdir(csv_outputpath_30s):
        mkdir(csv_outputpath_30s)

    #開啟影片檔轉為 WAV
    audioclip = VideoFileClip(f'{mp4_folder}/{file_name}.mp4').audio   # 先匯入影檔取出音檔
    audioclip.write_audiofile(f'{wav_outputpath}{file_name}.wav', fps=22050)  # 將取出的音檔寫入剛給的wav名稱及路經。
    audioclip.close()

    wav = AudioSegment.from_file(f'{wav_parentfolder}/{file_name}/{file_name}.wav', "wav") # 開啟wav檔案
    ...
    # 呼叫下方 librosa_feature_op 轉為 CSV
    librosa_feature_op(f'{wav_outputpath_3s}/', f'{csv_outputpath_3s}/{file_name}.csv')  # 3s.csv
    librosa_feature_op(f'{wav_outputpath_30s}/', f'{csv_outputpath_30s}/{file_name}.csv')  # 30s.csv
...
def librosa_feature_op(file_path, csv_path):
    ...
    file_data = []
    file_data = [f for f in listdir(file_path) if isfile (join(file_path, f)) ] # 此 file_path 為 def 傳入值檔案路徑

    door = True   #避免相同特徵遇到 'tempo'、 'beats' 、 'total' 而轉三次，
    for name in file_data:
        if re.findall(r'.wav+', name) == [".wav"]:  #抓出.wav檔案 對轉檔。
   
            y, sr = librosa.load(file_path+name) # 用 librosa讀取檔案
    ...
    for key ,value in feature_dir.items():
