...
def Video_to_csv(file_name):
    ...
    # 路徑設定 & 確認資料夾，沒有則建立
    # mp4 路徑 & 轉出 wav 音檔路徑
    mp4_folder = f'./musicfile/mp4/{file_name}'  # mp4 來源路徑
    wav_parentfolder = './musicfile/wav'        # 給 wav 輸出目錄的上一層目錄
    wav_outputpath = f'./musicfile/wav/{file_name}/'  # 給 wav 輸出目錄
    # 如果沒有資料夾建立資料夾
    ...
    # wav 分割成 3s 存放路徑
    wav_parentfolder_3s = './musicfile/wav/3s'
    wav_outputpath_3s = f'./musicfile/wav/3s/{file_name}'
    ...
    # wav 分割成 30s 存放路徑
    wav_parentfolder_30s = './musicfile/wav/30s'
    wav_outputpath_30s = f'./musicfile/wav/30s/{file_name}'
    ...
    # 特徵 csv 存放路徑 
    csv_folder ='./musicfile/csv'
    csv_parentfolder_3s = './musicfile/csv/3s'
    csv_parentfolder_30s = './musicfile/csv/30s'
    csv_outputpath_3s = f'./musicfile/csv/3s/{file_name}'
    csv_outputpath_30s = f'./musicfile/csv/30s/{file_name}'
